...
#ifndef DISABLE_H265
	for (const SdpVideoFormat& format : webrtc::SupportedH265Codecs())
    supported_codecs.push_back(format);
#endif
...
#ifndef DISABLE_H265
	if (cricket::CodecNamesEq(format.name, cricket::kH265CodecName))
    return H265Encoder::Create(cricket::VideoCodec(format));
#endif
