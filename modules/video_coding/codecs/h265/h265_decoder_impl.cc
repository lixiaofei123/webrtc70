...
int H265DecoderImpl::AVGetBuffer2(
    AVCodecContext* context, AVFrame* av_frame, int flags) {
  // Set in |InitDecode|.
  H265DecoderImpl* decoder = static_cast<H265DecoderImpl*>(context->opaque);
...
void H265DecoderImpl::AVFreeBuffer2(void* opaque, uint8_t* data) {
...
H265DecoderImpl::H265DecoderImpl() : pool_(true),
...
int32_t H265DecoderImpl::InitDecode(const VideoCodec* codec_settings,
  ...
  RTC_LOG(LS_ERROR) << "========================================";
  RTC_LOG(LS_ERROR) << "FFmpeg H.265 decoder init successful.";
  RTC_LOG(LS_ERROR) << "========================================";

  return WEBRTC_VIDEO_CODEC_OK;
}

int32_t H265DecoderImpl::Release() {
  av_context_.reset();
  av_frame_.reset();
  return WEBRTC_VIDEO_CODEC_OK;
}

int32_t H265DecoderImpl::RegisterDecodeCompleteCallback(
    DecodedImageCallback* callback) {
  decoded_image_callback_ = callback;
  return WEBRTC_VIDEO_CODEC_OK;
}

int32_t H265DecoderImpl::Decode(const EncodedImage& input_image,
                                bool /*missing_frames*/,
  ...
  //                  EncodedImage::GetBufferPaddingBytes(kVideoCodecH265));
  ...
  const ColorSpace& color_space = ExtractH265ColorSpace(av_context_.get());
...
void H265DecoderImpl::ReportInit() {
  if (has_reported_init_)
    return;
  RTC_HISTOGRAM_ENUMERATION("WebRTC.Video.H265DecoderImpl.Event",
                            kH265DecoderEventInit,
                            kH265DecoderEventMax);
  has_reported_init_ = true;
}

void H265DecoderImpl::ReportError() {
  if (has_reported_error_)
    return;
  RTC_HISTOGRAM_ENUMERATION("WebRTC.Video.H265DecoderImpl.Event",
                            kH265DecoderEventError,
                            kH265DecoderEventMax);
